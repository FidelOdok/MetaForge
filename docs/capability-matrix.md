...
| `twin` | `twin.constraint_violations` | List active constraint violations on a project. Scoped to the calling session's active project when one is set; with none, evaluates every constraint across every project (admin path) — previously always did this regardless of context, a real cross-project leak (FORGE-74) | [`tier1/twin-hp.md`](https://github.com/FidelOdok/MetaForge/blob/main/tests/uat/scenarios/tier1/twin-hp.md) |
