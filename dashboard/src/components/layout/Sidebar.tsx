...
export function Sidebar({ open = false, onClose }: { open?: boolean; onClose?: () => void }) {
  return <>
    ...
    <aside className={`workspace-sidebar ${open ? 'is-open' : ''}`}>
      <div className="brand-row"><NavLink to="/projects" className="brand" onClick={onClose}><img src="/favicon.svg" alt="" width="34" height="34" /><span>MetaForge<span className="brand-caption">ENGINEERING WORKSPACE</span></span></NavLink><button className="mobile-close icon-control" aria-label="Close navigation" onClick={onClose}><X size={20}/></button></div>
