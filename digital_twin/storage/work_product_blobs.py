"""Work-product blob storage in MinIO (MET-483, lifted here in MET-495).

Originally lived in ``api_gateway.twin.blob_store``; lifted to the
``digital_twin.storage`` layer so non-gateway callers (e.g. the
``twin.record_decision`` recorder used by the MCP sidecar) can store blobs
without importing the gateway. ``api_gateway.twin.blob_store`` re-exports
these for backward compatibility.

Work products (schematics, PCBs, gerbers, BOMs, PDFs, CAD, decision docs) are
stored under a ``work-products/{node_id}/{filename}`` key in the MinIO bucket —
the architecture's source of truth for work-product blobs.

Lazy + injectable: importing this module never requires ``minio``; the client
is built on first use from the ``MINIO_*`` environment. Tests pass a fake
``client``/``settings`` so no real ``minio`` install or server is needed.
"""
...
tracer = get_tracer("digital_twin.storage.work_product_blobs")
...
def work_product_object_key(node_id: str, filename: str) -> str:
    ...
    ``work-products/{node_id}/{filename}`` — node-scoped so every blob for a
    work product (and its revisions, by distinct filename) groups under one
    prefix in the bucket browser.
    """
...
def store_work_product_blob(
    ...
    *,
    ...
    Idempotent on the bucket (creates it if absent). ``client`` / ``settings``
